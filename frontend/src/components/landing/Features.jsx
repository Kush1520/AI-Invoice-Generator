      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {FEATURES.map((feature, index) => (
            <div
              key={index}
              className="group relative bg-white/60 dark:bg-zinc-900/40 backdrop-blur-md rounded-3xl p-8 border border-slate-200/50 dark:border-zinc-800/80 shadow-sm hover:shadow-md hover:border-slate-300 dark:hover:border-zinc-700 hover:-translate-y-1.5 transition-all duration-300"
              style={{ animationDelay: `${index * 0.1}s` }}
            >
              {/* Icon bubble */}
              <div className="mb-6 inline-flex items-center justify-center w-12 h-12 rounded-2xl bg-blue-55/40 dark:bg-zinc-800/50 group-hover:bg-blue-600 dark:group-hover:bg-blue-500 transition-colors duration-300">
                <feature.icon className="h-5 w-5 text-blue-600 dark:text-blue-400 group-hover:text-white transition-colors duration-300" />
              </div>

              {/* Index number */}
              <span className="absolute top-7 right-7 text-4xl font-black text-slate-100 dark:text-zinc-800/30 group-hover:text-blue-500/10 dark:group-hover:text-blue-900/10 transition-colors duration-300 select-none leading-none">
                0{index + 1}
              </span>

              <h3 className="text-base font-bold text-slate-900 dark:text-zinc-100 mb-2 leading-snug">
                {feature.title}
              </h3>
